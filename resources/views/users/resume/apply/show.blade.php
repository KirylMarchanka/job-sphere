@php use App\Components\Employer\Job\Invite\Enums\JobApplyTypeEnum; @endphp
@extends('layout.app')
...
@section('content')
    <div class="row">
        <div class="col-md-8">
            ...
            @if($apply->getRawOriginal('status') === 0 && $apply->getAttribute('type') === JobApplyTypeEnum::INVITE->value)
                <form id="chat-form" method="POST"
                      action="{{ route('users.invites-and-applies.update', ['apply' => $apply->getKey()]) }}">
                    @csrf
                    <div class="form-group mb-3">
                        <label for="status">Статус</label>
                        <select name="status" id="status"
                                @class(['form-control', 'is-invalid' => $errors->has('status')]) required>
                            @foreach([1 => 'Отклонить', 2 => 'Принять'] as $key => $status)
            ...
            @else
                <a href="{{ route('users.invites-and-applies.index') }}" class="btn btn-secondary">Назад</a>
            @endif
        </div>
    </div>
